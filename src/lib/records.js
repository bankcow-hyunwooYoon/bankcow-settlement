...
export function toCattleDetails(rows) {
  return rows.map((row) => ({
    ...
    사료비금액: Math.round(row.feedAmount),
    관리비사육일수: row.mgmtDays,
    관리비금액: Math.round(row.mgmtAmount),
    사료관리비합계: Math.round(row.feedAmount) + Math.round(row.mgmtAmount),
  }))
}

/** 정산월 + 총액으로 확정 월 데이터 한 건을 만든다. */
export function buildRecord({ 정산월, 사료비총액, 관리비총액, 상태, 등록일시 }) {
  if (상태 !== STATUS_CONFIRMED) {
    // 아직 계산 전이므로 소별 상세는 비워둔다.
    return { 정산월, 사료비총액, 관리비총액, 상태, 등록일시, 소별상세: null }
  }
...
/** 예외 개체를 위로, 그 아래 정상 개체를 개체 번호 순으로 정렬한다. */
export function sortRows(rows) {
  const exceptions = rows.filter((r) => r.status !== '정상').sort((a, b) => a.no - b.no)
  const normals = rows.filter((r) => r.status === '정상').sort((a, b) => a.no - b.no)
 ...
 */
export function getSelectableMonths(records, today = now()) {
  const savedMonths = new Set(records.map((r) => r.정산월))
  const startIndex = monthIndex(PRODUCT_START_MONTH)
