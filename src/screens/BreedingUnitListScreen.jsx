...
function FarmRegistrationModal({ products, unit, onClose, onSubmit }) {
  ...
  const canSubmit = farmName.trim().length > 0 && (isEdit || selectedProducts.length > 0)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="farm-registration-title">
      <div className={`max-h-[calc(100vh-2rem)] w-full overflow-hidden bg-white shadow-xl ${isEdit ? 'max-w-md' : 'max-w-2xl'}`}>
        <div className="border-b border-gray-200 px-5 py-4">
          <h2 id="farm-registration-title" className="text-base font-semibold text-gray-900">{isEdit ? '농장 정보 수정' : '농장 등록'}</h2>
          <p className="mt-1 text-xs text-gray-500">{isEdit ? '농장명만 수정할 수 있습니다.' : '농장명과 연결할 진행 중 투자상품을 선택해 주세요.'}</p>
        </div>
        ...
        <div className="flex justify-end gap-2 border-t border-gray-200 px-5 py-4">
          ...
          <button type="button" disabled={!canSubmit} onClick={() => onSubmit({ farmName: farmName.trim(), products: isEdit ? [] : selectedProducts })} className="bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 disabled:cursor-not-allowed disabled:bg-gray-300">{isEdit ? '저장' : '등록'}</button>
        </div>
      </div>
    </div>
  )
      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        {editingUnit && (
          <FarmRegistrationModal
            ...
            onSubmit={(payload) => {
              onUpdateUnit(editingUnit, payload)
              setEditingUnit(null)
            }}
          />
        )}
