...
export function CostSummaryCard({ totalFeedCost, totalMgmtCost, totalFeedDays, totalMgmtDays }) {
  const totalCost = totalFeedCost + totalMgmtCost
  const items = [
    { label: '사료비 배분 합계', value: totalFeedCost },
  ...
  return (
    <div className="mb-4 grid grid-cols-5 divide-x divide-gray-200 border border-gray-200 bg-white">
      {items.map((item) => (
        <div key={item.label} className="p-4">
          <p className="text-xs text-gray-500">{item.label}</p>
          <p className="mt-1 text-2xl font-semibold text-gray-900">
            {item.unit ? item.value.toLocaleString('ko-KR') : formatWon(roundWon(item.value))}
            ...
            return (
              <tr key={row.id} className={rowBg}>
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    {isException && (
                      <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${
                        row.isPastExit ? 'bg-gray-100 text-gray-400' : 'bg-red-100 text-red-700'
                      }`}>
                        {row.exitMonth.match(/(\d+)월/)?.[1]}월 이탈
                      </span>
                    )}
                  </span>
                </td>
                ...
                <td className="border-b border-gray-200 px-4 py-2.5">
                  <CattleStatusBadge status={row.status} muted={row.isPastExit} />
                </td>
...
export default function AllocationResult({ result }) {
  ...
  const sortedRows = useMemo(
    () => [...result.rows].sort((a, b) => a.no - b.no),
    [result.rows],
  )
